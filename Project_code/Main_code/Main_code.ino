...
void setSessionIDFromTime() {
    if (Firebase.getInt(fbdo,"/initialSessionID")) {
            int NewSessionID;
            NewSessionID = fbdo.to<int>();
            Serial.print("NEWSESSIONID : ");
            Serial.println(NewSessionID);
            sessionID= String(NewSessionID);
            NewSessionID++;
            sessionID="/DataCollection/Session_"+sessionID;
            Firebase.setInt(fbdo, "/initialSessionID", NewSessionID);
        } else {
            Serial.println("getInt failed: " + fbdo.errorReason());
        }
...
void bufferSensorData(float sensorValue) {
  if (dataCount < MAX_DATA_POINTS) {
    dataBuffer[dataCount].timestamp = millis(); 
    dataBuffer[dataCount].value = sensorValue;
    dataCount++;
  } else {
    return;
  }
...
bool sendBufferedData() {
  ...
  for (int i = 0; i < dataCount; i++) {
    String key = String(dataBuffer[i].timestamp);
    json.set(key.c_str(), dataBuffer[i].value);
  }
...
void setup() {
    ...
    // NTP Time sync with proper wait
...
void readSensorData() {
    ...
    // Apply direct assignment (Kalman filtering commented out)
    filteredAx = rawAx;
    filteredAy = rawAy;
    filteredAz = rawAz;
    filteredGx = rawGx;
    ...
    float net_acc = sqrt(filteredAx * filteredAx + 
                    filteredAy * filteredAy + 
                    filteredAz * filteredAz);

    bufferSensorData(net_acc);
}
...
void triggerFallAlert() {
    ...
    buzzAlert(3, 100);  // 3 buzzes, 100ms each
